...
impl<'info> Unstake<'info> {
  ...
  fn reward_to_staker(&mut self, bumps: &UnstakeBumps) -> Result<()> {
    ...
    transfer_checked(
      CpiContext::new_with_signer(self.reward_token_program.to_account_info(), 
       TransferChecked {
       ...
       signer_seeds
      ), reward_amount, self.reward_mint.decimals
    )?;
  ...
  fn update_stake_info(&mut self, amount: u64, bumps: &UnstakeBumps) -> Result<()> {
    ...
    if new_amount.eq(&0) {
      ...
      let signer_seeds = &[&seeds[..]];
      close_account(
        CpiContext::new_with_signer(self.stake_token_program.to_account_info(), CloseAccount { account: self.stake_info_ata.to_account_info(), destination: self.signer.to_account_info(), authority: self.stake_info.to_account_info() }, signer_seeds)
      )?;
